...
const preprocessorActions = {
    ...
    "@foreach": function (cc, list, method) {
        if (!list || !method) {
    ...
    "@if": function (cc, test, method, elseMethod) {
        if (!test || !method) {
    ...
    "@def": function (cc, name, args, cb) {
        if (!name || !args || !cb) {
            throw this.error(cc, "@def must have 3 arguments, name, [args...], @{cb}");
        }
    ...
    "@showinwidget": function (cc, setTo) {
        if (!setTo) {
