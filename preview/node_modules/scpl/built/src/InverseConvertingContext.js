...
const ESCAPEDQUOTEDSTRING = (value) => value
    .replace(/(["\\\n])/g, d => (d === "\n" ? "\\n" : `\\${d}`))
    .replace(/\r/g, "");
const DQUOTEDSTRING = (value) => `"${ESCAPEDQUOTEDSTRING(value)}"`;
const ESCAPESQUOTEDSTRING = (value) => value
    .replace(/(['\\\n])/g, d => (d === "\n" ? "\\n" : `\\${d}`))
    .replace(/\r/g, "");
...
class InverseConvertingContext {
    ...
    createActionsAble(value) {
        ...
        if (value.color) {
            res.unshift(`@Color ${ShortcutMeta_1.inverseColors[value.color]}`);
        }
        if (value.glyph) {
            res.unshift(`@Icon ${ShortcutMeta_1.inverseGlyphs[value.glyph]}`);
        }
        return res.join("\n");
    }
    createActionAble(value) {
        ...
        order.forEach(param => {
            ...
            result.push(`${param.readableName}=${this.handleArgument(OutputData_1.toParam(paramValue))}`);
        });
    ...
    createDictionaryAble(value) {
        const result = value.items.map(item => {
            ...
            if (typeof item.value === "boolean") {
                return `${key}: ??booleans are not supported yet in dictionaries in scpl. this boolean was ${item.value}??`;
            }
